if Backbone?
  class @DiscussionThreadListView extends Backbone.View
    events:
      ...
      "click .forum-nav-browse-filter-input": "ignoreClick"
      "click .forum-nav-browse-menu-item": "selectTopic"
    ...
    showBrowseMenu: =>
      if not @isBrowseMenuVisible()
        ...
        $("body").bind "click", @toggleBrowseMenu
    ...
    hideBrowseMenu: =>
      if @isBrowseMenuVisible()
        ...
        $("body").unbind "click", @toggleBrowseMenu
    ...
    fitName: (name) ->
      @maxNameWidth = (@$(".forum-nav-browse").width() * .8)
    ...
    selectTopic: (event) ->
      ...
      if item.hasClass("forum-nav-browse-menu-all")
        ...
        @$(".forum-nav-filter-cohort").toggle($(event.target).data('cohorted') == "True")
